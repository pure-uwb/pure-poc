...
public class Session implements PropertyChangeObservable {
    ...
    public Session(StateMachine stateMachine, PropertyChangeListener[] listeners) {
        this(stateMachine);
        for (PropertyChangeListener l : listeners) {
            this.addPropertyChangeListener(l);
        }
        notifyAllListeners("state", "NULL", stateMachine.getStateString());
    }
    ...
    public byte[] getRemoteTranscript() {
        ...
        try {
            outputStream.write(this.remoteKey.getEncoded());
            outputStream.write(this.localKey.getPublic().getEncoded());
            if (respTx != null & pollRx != null & finalTx != null) {
    ...
    public void setCardPublicKey(RSAPublicKey cardKey) {
