...
public class EmvKeyReader {
    ...
    public byte[] createNewCertificate(IssuerIccPublicKey issuerPublicKey, byte[] iccPublicKeyCertificate,
        ...
        final RecoveredIccPublicKey cert = this.parseIccPublicKeyCert(recoveredBytes,
                issuerPublicKey.getModulusBytes().length);
        cert.leftMostPubKeyDigits = newPublicKey;
        byte[] encoded = encodeIccPublicKeyCert(cert, null);
        return calculateRSA(encoded, issuerKey.getPrivateExponent(), issuerKey.getModulus());
    }
    ...
    byte [] encodeIccPublicKeyCert(RecoveredIccPublicKey cert, byte [] iccRemainingBytes)
        ...
        // Data. For simplicity in the extension we remove the statically authenticated data.
        ByteArrayOutputStream hashStream = new ByteArrayOutputStream();
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        outputStream.write((byte) 0x6a); //data header
        ...
        outputStream.write(hashStream.toByteArray(), 0, hashStream.size());
